...
def load_transactions():
    ...
    with open("/Users/randycostner/Desktop/transactions.csv", "r") as f:
        ...
        for row in reader:
            ...
            shares = (
                float(row["shares"]) if row["shares"] and row["shares"] != "NULL" else 0
            )
            price = (
                float(row["price"]) if row["price"] and row["price"] != "NULL" else 0
            )
            transactions.append(
                {
                    "date": date,
...
def load_and_filter_history():
    try:
        ...
        history_path = (
            "/Users/randycostner/Downloads/History_for_Account_211749109 (1).csv"
        )
        ...
        with open(history_path, "r", encoding="utf-8-sig") as f:
            ...
            lines = [
                line for line in f if line.strip() and not line.startswith('"The data')
            ]
            ...
            for row in reader:
                try:
                    ...
                    hist_type = (
                        "buy"
                        if "YOU BOUGHT" in row["Action"]
                        else "sell" if "YOU SOLD" in row["Action"] else "other"
                    )
                    ...
                    for trans in transactions:
                        if (
                            ...
                            and abs(trans["shares"] - abs(quantity))
                            < 0.01  # Use small threshold for float comparison
        ...
        if filtered_history:
            ...
            print(
                f"Successfully wrote {len(filtered_history)} entries to filtered history"
            )
    except Exception as e:
        print(f"Error: {str(e)}")
        print(
            f"Current row being processed: {row if 'row' in locals() else 'No row information available'}"
        )

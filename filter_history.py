        ...
        with open(history_path, 'r', encoding='utf-8-sig') as f:
            ...
            for row in reader:
                try:
                    ...
                    hist_type = 'buy' if 'YOU BOUGHT' in row['Action'] else 'sell' if 'YOU SOLD' in row['Action'] else 'other'
                    ...
                    for trans in transactions:
                        if (trans['date'].date() == date.date() and
                            trans['ticker'] == row['Symbol'] and
                            trans['type'] == hist_type and
                            abs(trans['shares'] - abs(quantity)) < 0.01 and  # Use small threshold for float comparison
                            (price == 0 or abs(trans['price'] - price) < 0.01)):  # Some prices might be 0 in transactions
                            found_match = True
                            break

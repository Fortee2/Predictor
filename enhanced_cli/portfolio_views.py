...
class RecalculatePortfolioValuesCommand(Command):
    ...
    def execute(self, cli, *args, **kwargs) -> None:
        ...
        if ui.confirm_action("Specify a start date for recalculation?"):
            while True:
                date_input = Prompt.ask("[bold]Start date[/bold] (YYYY-MM-DD format, or leave empty for earliest transaction)")
        ...
        if ui.confirm_action("Proceed with portfolio value recalculation?"):
            ...
            if result:
                ui.status_message("Portfolio values have been successfully recalculated!", "success")
            else:
                ui.status_message("Failed to recalculate portfolio values. Check the output above for details.", "error")
        else:

...
class rsi_calculations:
    ...
    def retrievePrices(self, id, ticker_id):
        ...
        try:
            with self.current_connection.cursor() as cursor:
                ...
                df = pd.DataFrame(cursor.fetchall(), columns=['close', 'activity_date', 'avg_loss', 'avg_gain', 'rs', 'rsi'])
    ...
    def calculateWeightedAverages(self, ticker_id):
        ...
        rsi_idx =  df_rs.columns.get_loc('rsi') #relative strength index (Converts RS to a value between 0 and 100)
        ...
        for rsi in range(array_len):
            ...
            if rsi == 13 and id == 1:  # We only want to hit here if no history has been calculated
                df_rs.iloc[13, avgGain_idx] = round(Decimal(df_rs.iloc[0:13, gain_idx].mean()), 2)
                df_rs.iloc[13, avgLoss_idx] = round(Decimal(df_rs.iloc[0:13, loss_idx].mean()), 2)
            elif rsi > 13 or id > 1:  # Only want to hit here if we have done seed calculation or we have past history

...
class MACD:
    ...
    def load_macd_from_db(self, ticker_id):
        ...
        df = pd.DataFrame(cursor.fetchall(), 
                         columns=['activity_date', 'macd', 'signal_line'])
        df = df.set_index('activity_date')
    ...
    def get_macd_signals(self, ticker_id):
        ...
        for date in df.index[1:]:  # Skip first row due to shift
            ...
            if df.loc[date, 'macd'] > df.loc[date, 'signal_line'] and \
               df.loc[date, 'macd_shift'] <= df.loc[date, 'signal_shift']:
                signals.append({
                    'date': date,
                    'signal': 'BUY',
            ...
            elif df.loc[date, 'macd'] < df.loc[date, 'signal_line'] and \
                 df.loc[date, 'macd_shift'] >= df.loc[date, 'signal_shift']:
                signals.append({
                    'date': date,
                    'signal': 'SELL',

    ...
    def loadAveragesFromDB(self, ticker_id, averageType):
        ...
        try:
            with self.get_connection() as connection:
                ...
                logger.debug(f"Loaded {len(df)} average data points for ticker {ticker_id}, type {averageType}")
                return df
        except mysql.connector.Error as e:
            logger.error(f"Database error loading averages for ticker {ticker_id}, type {averageType}: {str(e)}")
            raise
        except Exception as e:
        ...
        try:
            with self.get_connection() as connection:
                ...
                if last_date is None:
                    ...
                    logger.debug(f"No previous moving averages found for ticker {ticker_id}, calculating all")
                else:
                    last_date = last_date - timedelta(days=period)
                    logger.debug(f"Last moving average date for ticker {ticker_id}: {last_date}")
                ...
                if not new_data.empty:
                    logger.info(f"Found {len(new_data)} new data points to calculate moving averages")
                    ...
                    connection.commit()
                    logger.info(f"Updated {rows_updated} moving averages for ticker {ticker_id}")
                else:

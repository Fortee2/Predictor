...
from data.fifo_cost_basis_calculator import \
    calculate_fifo_position_from_transactions
        ...
        try:
            ...
            for ticker_id, ticker_transactions in transactions_by_ticker.items():
                try:
                    ...
                    print(
                        f"Warning: Error calculating FIFO position for {ticker_symbols.get(ticker_id, ticker_id)}: {e}"
                    )
            ...
            stock_prices = {}
            # Only fetch market data for tickers with positive positions
                ...
                try:
                    ...
                    if hist_data.empty:
                        ...
                        if len(valid_dates) > 0:
                            ...
                            stock_prices[ticker_id] = close_price
                            print(
                                f"  {symbol}: No prior data, using earliest available from {first_date.date()}: ${close_price:.2f}"
                            )
                except Exception as e:
